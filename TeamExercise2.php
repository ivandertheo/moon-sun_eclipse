...
<head>
  <meta charset="utf-8" />
  <title>Virtual Gallery — Camera Planner</title>
  <style>
    html, body {
      margin: 0;
      height: 100%;
...
<body>
  ...
  <div id="labels">
    Kiri: Perspective • Kanan: Orthographic (top-down) + CameraHelper
  </div>
  ...
  <script type="module">
    ...
    let renderer, scene, camPersp, camOrtho, controlsPersp, helper;
    ...
    function init() {
      ...
      const floor = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({ color: 0x505050, roughness: 1.0 })
      );
      ...
      const bright = hex => new THREE.MeshStandardMaterial({
        color: hex, metalness: 0.2, roughness: 0.4
      });

      const statueA = new THREE.Mesh(new THREE.TorusKnotGeometry(0.5, 0.18, 120, 16), bright(0x9ed1ff));
      statueA.position.set(-2, -2.2, -1);
      statueA.castShadow = true;
      scene.add(statueA);

      const statueB = new THREE.Mesh(new THREE.SphereGeometry(0.8, 48, 32), bright(0xffb6d5));
      statueB.position.set(0, -2.2, 2);
      statueB.castShadow = true;
      scene.add(statueB);

      const statueC = new THREE.Mesh(new THREE.ConeGeometry(0.7, 1.8, 48), bright(0xc7ffb6));
      ...
      const aspect = 1;
      const orthoH = 6;
      camOrtho = new THREE.OrthographicCamera(-orthoH * aspect, orthoH * aspect, orthoH, -orthoH, 0.1, 200);
      ...
      function setShot(kind, target) {
        ...
        if (kind === 'close') {
          camPersp.fov = 35; pos.set(0.4, 0.25, 1.4);
        } else if (kind === 'medium') {
          camPersp.fov = 55; pos.set(1.2, 0.8, 3.2);
        } else {
          camPersp.fov = 75; pos.set(2.2, 1.0, 6.0);
        }
        camPersp.position.copy(target).add(pos);
    ...
    function onResize() {
      ...
      const aspectR = Math.max(1e-6, (width * 0.5) / height);
      const orthoH = 6;
      camOrtho.left   = -orthoH * aspectR;
      camOrtho.right  =  orthoH * aspectR;
      camOrtho.top    =  orthoH;
